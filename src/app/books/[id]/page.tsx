...
export default function Page({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    async function ShowBook() {
      try {
        const response = await axios.get<GoogleBook>(`https://www.googleapis.com/books/v1/volumes/${params.id}`);
        ...
        {book?.volumeInfo && (
          <div>
            <div>
              ...
              <p>{book.volumeInfo.authors?.join(", ") ?? "Author information not available."}</p>
              <p>{book.volumeInfo.publishedDate ?? "Publication date not available"}</p>
